...
get '/debug' => sub {
    ...
    my $auth_source = $c->session('auth_source') // 'oauth';
    my $breakglass_role = $c->session('breakglass_role') // '';
    ...
    $c->render(json => {
        ...
        breakglass_role => $breakglass_role,
...
sub load_creds {
    ...
    my $ldap_group_base = $ENV{LDAP_GROUP_BASE} || 'ou=groups,ou=winterbourne,dc=critchley,dc=biz';
        ...
        my $breakglass_ops_user = $ENV{BREAKGLASS_OPS_USER} || 'breakglass';
        my $breakglass_audit_user = $ENV{BREAKGLASS_AUDIT_USER} || 'breakglass_audit';
        my $breakglass_ops_user = $ENV{BREAKGLASS_OPS_USER} || 'breakglass';
        my $breakglass_audit_user = $ENV{BREAKGLASS_AUDIT_USER} || 'breakglass_audit';
    my $breakglass_ops_user = $ENV{BREAKGLASS_OPS_USER} || 'breakglass'; 
    my $breakglass_audit_user = $ENV{BREAKGLASS_AUDIT_USER} || 'breakglass_audit'; 
    ...
    for my $i (0..$#ARGV) {
        ...
        if ($ARGV[$i] eq '--breakglass-ops-user' && defined $ARGV[$i + 1]) {
            $breakglass_ops_user = $ARGV[$i + 1];
        }
        if ($ARGV[$i] eq '--breakglass-audit-user' && defined $ARGV[$i + 1]) {
            $breakglass_audit_user = $ARGV[$i + 1];
        }
        if ($ARGV[$i] eq '--breakglass-ops-user' && defined $ARGV[$i + 1]) {
            $breakglass_ops_user = $ARGV[$i + 1];
        }
    ...
    $data->{breakglass_ops_user} = $breakglass_ops_user;
    $data->{breakglass_audit_user} = $breakglass_audit_user;
    $data->{breakglass_ops_user} = $breakglass_ops_user;
    $data->{breakglass_audit_user} = $breakglass_audit_user;
...
sub check_auth {
    ...
    if (($c->session('auth_source') // '') eq 'breakglass' && $c->session('email')) {
        return 1;
    }
...
sub route_breakglass_auth {
    ...
    if ($res->is_success) {
        my $role = 'viewer';
        $role = 'breakglass_ops' if lc($user) eq lc($creds->{breakglass_ops_user} || 'breakglass');
        $role = 'breakglass_audit' if lc($user) eq lc($creds->{breakglass_audit_user} || 'breakglass_audit');

        $c->session(email => "breakglass:$user");
        $c->session(name => "Breakglass $user");
        $c->session(auth_source => 'breakglass');
        $c->session(breakglass_user => $user);
        $c->session(breakglass_role => $role);
...
sub route_home {
    ...
    if (($c->session('auth_source') // '') eq 'breakglass') {
        my $email = $c->session('email') // 'unknown';
        my $role = $c->session('breakglass_role') // 'viewer';
        return $c->render(text =>
            "Welcome (break-glass session).\n" .
            "Identity: $email\n" .
            "Role: $role\n" .
            "Use /ops for operations controls.\n"
        );
    }
...
<body>
    ...
    <div class="panel">
        ...
        <p><strong>LDAP groups:</strong> %s</p>
...
sub ops_status {
    ...
    my $email = $c->session('email') // '';
    my $auth_source = $c->session('auth_source') // 'oauth';
    my $session_breakglass_role = $c->session('breakglass_role') // '';
    my $groups = $c->session('ldap_groups') || [];
...
sub ops_audit_log_path {
    return $ENV{OPS_AUDIT_LOG} || "$RealBin/ops_audit.log.jsonl";
}
